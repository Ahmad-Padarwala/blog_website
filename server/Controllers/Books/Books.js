...
//get books with limits
const getBooksWithLimits = async (req, res) => {
  const limits = parseInt(req.params.limit);
  const q =
    "select *, DATE_FORMAT(DATE(book_publish_date), '%Y-%m-%d') AS upload_date FROM `bg_books_detail` where  book_status = ? order by id desc limit ? ";

  const flag = true;
  conn.query(q, [flag, limits], (err, data) => {
    if (err) {
      console.log(err);
    } else {
      res.send(data);
...
// get Books with filter
const getAllBookCateFilter = (req, res) => {
  const id = req.params.id;
  if (id == 0) {
    const q =
      "select *, DATE_FORMAT(DATE(book_publish_date), '%Y-%m-%d') AS upload_date FROM `bg_books_detail` where  book_status = ? order by id desc ";
    ...
    const sql = `SELECT * FROM bg_books_detail WHERE books_category =? AND book_status= 1 order by id desc`;
    conn.query(sql, id, (err, data) => {
      if (err) {
        console.log(err);
      } else {
