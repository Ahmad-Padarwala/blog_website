...
var imgconfig = multer.diskStorage({
  destination: (req, file, callback) => {
    callback(null, "../client/public/upload/blog");
  },
...
router.get("/getblogpostswithcategory/:category", (req, res) => {
  ...
  if (category == 0) {
    ...
    const q =
      "SELECT *, DATE_FORMAT(DATE(blog_publish_date), '%d-%m-%Y') AS blog_publish_date FROM bg_blog_post WHERE blog_status = ? AND blog_category = ? AND blog_delete_status = ? ORDER BY id DESC";

    const flag = true;
    const remove = true;

    conn.query(q, [flag, category, remove], (err, data) => {
      if (err) {
        console.log(err);
        res.status(500).json({ error: "Internal Server Error" });
      } else {
